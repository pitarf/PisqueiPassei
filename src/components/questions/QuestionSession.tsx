...
export const QuestionSession: React.FC<QuestionSessionProps> = ({ initialQuestions, title }) => {
  ...
  const handleSubmitAnswer = async () => {
    if (!selectedOption || !currentQuestion || isSubmitting) {
      if (!selectedOption) toast.info("Selecione uma alternativa de A a E para responder.");
      return;
    }
  ...
  if (isFinished) {
    ...
    return <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 text-center space-y-5 max-w-xl mx-auto shadow-xl"><div className="w-16 h-16 rounded-full bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center text-emerald-400 mx-auto"><Award className="w-8 h-8" /></div><div><h2 className="text-xl sm:text-2xl font-black text-white">Treino Concluído!</h2><p className="text-xs sm:text-sm text-slate-400 mt-1">Seu desempenho foi registrado e o domínio dos assuntos foi recalculado.</p></div><div className="grid grid-cols-3 gap-3 bg-slate-800/80 p-4 rounded-xl border border-slate-700/60"><div><p className="text-[11px] text-slate-400">Respondidas</p><p className="text-xl font-bold text-white">{total}</p></div><div><p className="text-[11px] text-slate-400">Acertos</p><p className="text-xl font-bold text-emerald-400">{correctCount}</p></div><div><p className="text-[11px] text-slate-400">Aproveitamento</p><p className="text-xl font-bold text-sky-400">{accuracy}%</p></div></div><div className="flex items-center justify-center gap-3 pt-3"><Link href="/questoes" className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-xs">Nova Bateria</Link><Link href="/" className="px-5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs shadow-md">Voltar ao Início</Link></div></div>;
  }
