...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      flashcards: due.slice(0, RESPONSE_LIMIT),
      dueCount: due.length,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const recentReview = await tx.flashcardReview.findFirst({
        where: {
          userId: user.id,
          flashcardId,
          rating,
          reviewedAt: { gte: new Date(now.getTime() - DUPLICATE_WINDOW_MS) },
        },
        orderBy: { reviewedAt: "desc" },
      });

      if (recentReview) {
        return { review: recentReview, duplicate: true };
      }
      ...
      await tx.user.update({
        where: { id: user.id },
        data: { xp: { increment: 5 }, lastStudyDate: now },
      });
