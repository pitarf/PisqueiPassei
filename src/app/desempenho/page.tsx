...
export default async function DesempenhoPage() {
  ...
  return <div className="space-y-6">
    <header className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-md">
      ...
      <p className="text-xs sm:text-sm text-slate-400 mt-1">Acompanhe seu ritmo de estudo e evolução nas questões. Os dados refletem seu treino e ajudam a priorizar o que revisar.</p>
    </header>
    ...
    <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
      <div className="flex items-start justify-between gap-3"><div><h2 className="text-base font-bold text-white flex items-center gap-2"><BarChart2 className="w-4 h-4 text-emerald-400" />Desempenho por matéria</h2><p className="text-xs text-slate-400 mt-0.5">Nível de domínio e taxa de acerto em cada matéria do edital.</p></div><Link href="/edital" className="text-xs font-semibold text-emerald-400">Ver edital</Link></div>
    ...
    <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
      <h2 className="text-base font-bold text-white flex items-center gap-2"><TrendingUp className="w-4 h-4 text-sky-400" />Últimas questões respondidas</h2>
      <p className="text-xs text-slate-400 mt-0.5">Seu rendimento nas últimas {recentTraining.length} questões de treino.</p>
      <div className="grid grid-cols-3 gap-3 mt-4"><div className="bg-slate-800/70 rounded-xl p-3"><span className="text-[11px] text-slate-400">Aproveitamento recente</span><p className="text-xl font-black text-sky-300 mt-1">{recentAccuracy}%</p></div><div className="bg-slate-800/70 rounded-xl p-3"><span className="text-[11px] text-slate-400">Acertos</span><p className="text-xl font-black text-emerald-300 mt-1">{recentTraining.filter(a=>a.isCorrect).length}</p></div><div className="bg-slate-800/70 rounded-xl p-3"><span className="text-[11px] text-slate-400">Erros</span><p className="text-xl font-black text-rose-300 mt-1">{recentTraining.filter(a=>!a.isCorrect).length}</p></div></div>
    </section>

    <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
      <div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-bold text-white flex items-center gap-2"><Target className="w-4 h-4 text-rose-400" />Tópicos para reforçar</h2><p className="text-xs text-slate-400 mt-0.5">Assuntos com domínio abaixo de 70% onde você mais errou.</p></div><Link href="/questoes?modo=erros" className="text-xs font-semibold text-rose-300">Treinar erros</Link></div>
      {weak.length ? <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4">{weak.map(p => { const topicAttempts = training.filter(a => a.question.topicId === p.topicId); const topicErrors = topicAttempts.filter(a => !a.isCorrect).length; return <Link key={p.id} href={`/aula/${p.topicId}`} className="bg-slate-800/70 border border-slate-700/60 rounded-xl p-3 hover:border-rose-500/40 transition-colors block min-w-0"><div className="flex items-center justify-between gap-2 min-w-0"><span className="text-xs font-semibold text-slate-200 truncate flex-1">{p.topic.code ? `${p.topic.code} ` : ""}{p.topic.title}</span><span className="text-xs font-black text-rose-300 shrink-0">{Math.round(p.masteryScore)}%</span></div><div className="text-[10px] text-slate-500 mt-1 truncate">{topicAttempts.length ? `${topicErrors} erro(s) em ${topicAttempts.length} questão(ões)` : "Sem questões respondidas neste tópico"}</div></Link>; })}</div> : <p className="text-xs text-slate-400 mt-4">Nenhum tópico com domínio baixo no momento.</p>}
    </section>

    <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
      <h2 className="text-base font-bold text-white flex items-center gap-2 mb-3"><Award className="w-4 h-4 text-amber-400" />Histórico de simulados</h2>
      {user?.simulations.length ? <div className="space-y-2">{user.simulations.map((sim,i) => { const d=sim.detailsJson&&typeof sim.detailsJson==="object"&&!Array.isArray(sim.detailsJson)?sim.detailsJson as {isEliminated?:boolean;eliminationReasons?:string[];targetScore?:number}:null; const eliminated=d?.isEliminated===true; const target=d?.targetScore??47; return <div key={sim.id} className="bg-slate-800/70 rounded-xl p-3.5 border border-slate-700/60 flex items-center justify-between gap-3"><div><p className="text-xs font-bold text-slate-200">Simulado #{user.simulations.length-i}</p><p className="text-[11px] text-slate-400">{new Date(sim.completedAt).toLocaleDateString("pt-BR")}</p></div><div className="text-right"><span className="text-lg font-black text-white">{Math.round(sim.score)}/60</span><p className={`text-[10px] font-bold ${eliminated?"text-rose-300":"text-slate-400"}`}>{eliminated?"ELIMINADO":`Sua meta: ${target}/60`}</p></div></div>; })}</div> : <p className="text-xs text-slate-400 py-3 text-center">Você ainda não completou nenhum simulado.</p>}
    </section>
  </div>;
}
