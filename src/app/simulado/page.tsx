...
export default async function SimuladoPage({ searchParams }: SimuladoPageProps) {
  ...
  if (iniciar === "true") {
    ...
    const [portPool, mathPool, specificPool] = await Promise.all([
      portuguese ? prisma.question.findMany({ where: { topic: { subjectId: portuguese.id } }, include: { topic: { include: { subject: true } } } }) : [],
      math ? prisma.question.findMany({ where: { topic: { subjectId: math.id } }, include: { topic: { include: { subject: true } } } }) : [],
      prisma.question.findMany({ where: { topic: { subjectId: { in: specificIds } } }, include: { topic: { include: { subject: true } } } }),
    ]);
    const questions = [...shuffle(portPool).slice(0, 10), ...shuffle(mathPool).slice(0, 10), ...shuffle(specificPool).slice(0, 40)];
    if (questions.length < 60) return <div className="max-w-2xl mx-auto bg-slate-900 border border-rose-800/60 rounded-2xl p-6 text-center space-y-3">
      ...
      <p className="text-sm text-slate-400">Temos {questions.length} de 60 questões cadastradas para este formato. Para manter as regras reais do edital (10 de Português, 10 de Matemática e 40 de Específicas), o simulado completo será liberado assim que o banco atingir a cota.</p>
  ...
  return <div className="space-y-6 pb-20 sm:pb-6">
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-md">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Link href="/simulado?iniciar=true" className="px-6 py-3.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-sm shadow-lg flex items-center justify-center gap-2">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
        <p className="text-xs font-semibold text-slate-400 uppercase">Língua Portuguesa</p>
        <p className="text-2xl font-black text-white mt-1">10 Questões</p>
        <p className="text-[11px] text-slate-400 mt-1">Conhecimentos Básicos • Elimina se zerar a matéria</p>
      </div>
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
        <p className="text-xs font-semibold text-slate-400 uppercase">Matemática</p>
        <p className="text-2xl font-black text-white mt-1">10 Questões</p>
        <p className="text-[11px] text-slate-400 mt-1">Conhecimentos Básicos • Elimina se zerar a matéria</p>
      </div>
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
        <p className="text-xs font-semibold text-slate-400 uppercase">Conhecimentos Específicos</p>
        <p className="text-2xl font-black text-emerald-400 mt-1">40 Questões</p>
        <p className="text-[11px] text-slate-400 mt-1">Ênfase 18 • Exige no mínimo 50% de acerto</p>
      </div>
    </div>
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-bold text-white flex items-center gap-2"><Award className="w-4 h-4 text-amber-400" />Histórico de Provas</h2>
        <span className="text-xs text-slate-400">{user?.simulations?.length || 0} realizadas</span>
      </div>
      {user?.simulations?.length ? <div className="divide-y divide-slate-800">
        {user.simulations.map((sim) => <div key={sim.id} className="py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h3 className="text-sm font-bold text-slate-100">{sim.title}</h3>
            <div className="flex items-center gap-3 text-xs text-slate-400 mt-1">
              <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />{new Date(sim.completedAt).toLocaleDateString("pt-BR")}</span>
              <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{Math.round(sim.durationSeconds / 60)} min de prova</span>
            </div>
          </div>
          <div className="text-right">
            <span className="text-xl font-black text-white">{Math.round(sim.score)}</span>
            <span className="text-xs text-slate-400"> / 60</span>
            <div className="text-[10px] text-slate-400">Sua meta: {user.targetScore}/60</div>
          </div>
        </div>)}
      </div> : <div className="text-center py-10 border border-dashed border-slate-800 rounded-xl">
        <p className="text-sm text-slate-300">Nenhum simulado realizado até o momento.</p>
        <p className="text-xs text-slate-500 mt-1">Assim que você fizer sua primeira prova de 4 horas, o histórico completo aparecerá aqui.</p>
      </div>}
    </div>
  </div>;
}
