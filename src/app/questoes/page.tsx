  ...
  return <div className="space-y-6">
    {!isTrainingActive || questions.length === 0 ? <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between"><div><span className="text-xs font-bold uppercase tracking-wider text-rose-400 flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5" /> Modo Pontos Fracos</span><h3 className="text-base font-bold text-white mt-1.5">Treinar Meus Erros</h3><p className="text-xs text-slate-400 mt-1">Prioriza questões que você realmente errou. Se ainda não houver erros registrados, usa os tópicos com menor domínio como fallback.</p></div><div className="mt-4 pt-3 border-t border-slate-800 flex items-center justify-between"><span className="text-[11px] text-slate-400">até 10 questões</span><Link href="/questoes?modo=erros&count=10" className="px-4 py-2 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 text-rose-300 font-bold text-xs transition-colors">INICIAR TREINO</Link></div></div>
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between"><div><span className="text-xs font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" /> Bateria Geral</span><h3 className="text-base font-bold text-white mt-1.5">Treino Misto (20 Questões)</h3><p className="text-xs text-slate-400 mt-1">Bateria com questões sortidas de Conhecimentos Gerais e Específicos, em proporção aproximada ao formato da prova.</p></div><div className="mt-4 pt-3 border-t border-slate-800 flex items-center justify-between"><span className="text-[11px] text-slate-400">20 questões mistas</span><Link href="/questoes?modo=geral&count=20" className="px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs transition-colors shadow-sm">INICIAR TREINO</Link></div></div>
      </div>
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5"><h3 className="text-sm font-bold uppercase tracking-wider text-slate-300 mb-3 flex items-center gap-2"><Filter className="w-4 h-4 text-emerald-400" /> Praticar por Eixo do Edital</h3><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">{subjects.map((subj) => <div key={subj.id} className="bg-slate-800/70 border border-slate-700/60 rounded-xl p-3.5 flex flex-col justify-between"><div><h4 className="text-xs font-bold text-white">{subj.name}</h4><p className="text-[11px] text-slate-400 mt-0.5">{subj.topics.length} tópicos cadastrados</p></div><div className="mt-3 pt-2 border-t border-slate-700/50 flex items-center justify-between"><Link href={`/questoes?subjectId=${subj.id}&count=10`} className="text-xs font-semibold text-emerald-400 hover:text-emerald-300">10 Questões →</Link><Link href={`/questoes?subjectId=${subj.id}&count=20`} className="text-xs font-semibold text-slate-400 hover:text-slate-200">20 Questões</Link></div></div>)}</div></div>
    </div> : <div className="space-y-3">
      {isShortBattery && <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-3 flex gap-3"><Info className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" /><p className="text-xs text-amber-200">O filtro pediu <strong>{countNum}</strong> questões, mas o banco disponível retornou <strong>{questions.length}</strong>. A bateria seguirá com o que existe, sem inventar questões oficiais.</p></div>}
