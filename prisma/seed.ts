...
async function main() {
  ...
  await prisma.systemSetting.upsert({
    where: { key: "examDate" },
    update: { value: "2026-12-06T00:00:00.000Z" },
    create: { key: "examDate", value: "2026-12-06T00:00:00.000Z" },
  });
  ...
  for (const sub of subjectsData) {
    ...
    if (!existingSubject) {
      existingSubject = await prisma.subject.create({ data: { name: sub.name, category: sub.category, order: sub.order } });
    }
    ...
    for (const top of sub.topics) {
      ...
      await prisma.userTopicProgress.upsert({
        where: { userId_topicId: { userId: user.id, topicId: existingTopic.id } },
        update: {},
        create: { userId: user.id, topicId: existingTopic.id, status: "NAO_INICIADO", masteryScore: 0 },
      });
    }
  }
